...
from .models import Author,Picture,Category,Location,Comments
from .forms import SignUpForm,NewPostForm,EditProfileForm,PostComments
...
def single_pic(request,id):
    arts = Picture.objects.get(id=id)
    comments = Comments.get_comment_by_image(id = id)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            comment.save()
            return redirect('single-pic', id = id )

    else:
        form = PostComments()
    return render(request, 'single_pic.html', {"pic":pic,'arts':arts,'form':form, 'comments':comments})
...
#     arts = Picture.objects.get(id=art_id)
